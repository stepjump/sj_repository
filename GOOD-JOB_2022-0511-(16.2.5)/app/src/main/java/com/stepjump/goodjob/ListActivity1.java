...
public class ListActivity1 extends AppCompatActivity {
    ...
    @Override
        public void onCreate(Bundle savedInstanceState) {
            ...
            rv_message = intent.getExtras().getString("snd_message");    // MainActivity1 에서 넘어온값
...
//                                startActivity(intent);
                    ...
                    fiM = Integer.parseInt(fsM);    // month 문자를 정수로 변환 - 오류??????
                    ...
                    RequestQueue queue = Volley.newRequestQueue(ListActivity1.this);
    ...
    public void ListView(String rv_message) {   // ListView 보기
        ...
        listview = (ListView) findViewById(R.id.listview1);  // ListActivity1, ListActivity2   activity_list2.xml 같이 사용중.
...
//                    String result = "";
                    ...
                    for (int i = 0; i < ja.length(); i++) {
                        ...
                        String today_money = Integer.toString(order.getInt("today_money"));
                        String end_yn = order.getString("end_yn");
                        String reg_time = order.getString("reg_time");
                        String active_list = order.getString("active_list");
                        String commit_date = order.getString("commit_date");
                        ...
                        if (success) {
                            ...
                            num =  Integer.parseInt(today_money);

                            DecimalFormat df = new DecimalFormat("#,###");
                            today_money =  df.format(num) + "원";
                            // ===============================================================================================================================================

                            // ===============================================================================================================================================
                            // 'Y' 를 '정산완료' 로 만들기
                            if(end_yn.equals("Y")) {
                                end_yn = "정산완료" + "[" + commit_date + "]";
                            }
                            else {
                                end_yn = "미정산";
                            }
                            ...
                            //리스트뷰에 보여질 아이템을 추가
//                            list.add(list_item);
                            adapter.addItem(exe_date, today_money, end_yn, reg_time, active_list) ;


                        } else { // 가져오기에 실패한 경우
                            Toast.makeText(getApplicationContext(), "착한일 내역 가져오기에 실패했습니다.", Toast.LENGTH_SHORT).show();
                            return;
                        }
                    }
        } catch (
...
//                        boolean success = order.getBoolean("success");
//                        String exe_date = order.getString("exe_date");
//                        //                        String today_money = order.getString("today_money");
//                        String today_money = Integer.toString(order.getInt("today_money"));
//                        String end_yn = order.getString("end_yn");
//                        String reg_time = order.getString("reg_time");
//                        String active_list = order.getString("active_list");
//
//                        list_item = "날짜[" + exe_date + "] - 수입[" + today_money + "] - 결재여부[" + end_yn + "] - 등록일[" + reg_time + "] -  착한일목록[" + active_list + "]";
//
//                        if (success) {
//
//                            //리스트뷰에 보여질 아이템을 추가
